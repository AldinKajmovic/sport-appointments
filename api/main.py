...
def addPlayerToTeam(req: Request,team_name:str, db: Session = Depends(get_db)):
    userID = getUserID(req, db)    
    team = db.query(Team).filter(Team.name == team_name).first()
    lastID = db.query(TeamPlayer).order_by(desc(TeamPlayer.team_player_id)).first()
    new_player = TeamPlayer(
        user_id=userID.user_id,
        team_id=team.team_id, 
        team_player_id=lastID.team_player_id + 1 if lastID else 1  
    )
    db.add(new_player)
    db.commit()
...
@app.post("/add_player_to_team/{team_name}")
def mainFunction(req: Request,team_name:str,  db: Session = Depends(get_db)):
    return addPlayerToTeam(req,team_name,db)
...
def search_teams(name: Optional[str] = None, leader: Optional[str] = None, sport_name: Optional[str] = None, db: Session = Depends(get_db)):

    query = db.query(Team).options(
        joinedload(Team.users),
        joinedload(Team.sports),
    ...
    if leader:
        query = query.join(User).filter(User.username == leader)
        
    if sport_name:
        query = query.join(Sport).filter(Sport.name == sport_name)

    teams = query.all()

    results = []
    for team in teams:
        leader_user = db.query(User).filter(User.user_id == team.leader).first()

        team_data = {
            "team_id": str(team.team_id),
            "name": team.name,
            "leader": leader_user.username if leader_user else None,
            "sport": team.sports.name,
            "foundation_date": team.foundation_date.isoformat(),
            "team_players": [{"id": str(player.user_id), "username": player.users.username} for player in team.team_players]
        }
...
def search_teams_route(name: Optional[str] = None, leader: Optional[str] = None, sport: Optional[str] = None,  db: Session = Depends(get_db)):
    teams = search_teams(name,leader, sport, db)
    if not teams:
        raise HTTPException(status_code = 404, detail = "No areas found")
    return teams
...
def getFilterDataTeam(db: Session = Depends(get_db)):
             
        sports = db.query(Sport.name).distinct().all()
        sport_list = [sport[0] for sport in sports]

        leaders = db.query(User.username).join(Team, Team.leader == User.user_id).distinct().all()
        leaders_list = [leader[0] for leader in leaders]
...
@app.get("/filter_data_team")
def get_filter_data_team(db: Session = Depends(get_db)):
    data = getFilterDataTeam(db)
    return data

def getFilterDataAppointments(db: Session = Depends(get_db)):
       
        areas = db.query(Area.name).distinct().all()
...
@app.get("/filter_data_appointments")
def get_filter_data_appointments(db: Session = Depends(get_db)):
    data = getFilterDataAppointments(db)
    return data
...
def getUserTeams(request: Request, db: Session = Depends(get_db)):
    ...
    teams = db.query(Team).join(TeamPlayer).filter(TeamPlayer.user_id == user.user_id).options(
        joinedload(Team.team_players).joinedload(TeamPlayer.users) 
    ).all()
...
def searchAppointments(request:Request, db: Session = Depends(get_db)) -> List[dict]:
    userID = getUserID(request, db)
    jmbg = userID.user_id

    user_team_ids = db.query(TeamPlayer.team_id).filter(TeamPlayer.user_id != jmbg).distinct().all()

    user_teams = db.query(TeamPlayer.team_id).filter(TeamPlayer.user_id == jmbg).distinct().all()
    user_teams_set = {team_id[0] for team_id in user_teams}  
    unique_team_ids = {team_id[0] for team_id in user_team_ids} 
    filtered_team_ids = unique_team_ids - user_teams_set  
    ...
    upcoming_appointments = db.query(Appointment).filter(
        and_(
            ...
            Appointment.team_id.in_(filtered_team_ids)
        )
    ...
    for appointment in upcoming_appointments:
        ...
        team = db.query(Team).filter(Team.team_id == appointment.team_id).first()
        team_name = team.name if team else "Nepoznati tim"
    ...
    if not appointment_info:
        raise HTTPException(status_code=404, detail="Nijedan budući termin nije pronađen za ovog korisnika")
...
def search_appointments(request:Request, db: Session = Depends(get_db)):
    return searchAppointments(request,db)
...
def getAllTeams(req:Request,db: Session = Depends(get_db)):
    user = getUserID(req,db)
    teams = db.query(Team).all()
    return teams
